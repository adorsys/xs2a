...
class SinglePaymentTypeValidatorImplTest {
    ...
    @Test
    void doValidation_remittance_unstructured_reference_error() {
        singlePayment.setRemittanceInformationUnstructuredArray(Collections.singletonList(VALUE_141_LENGTH));
    ...
    @Test
    void doSingleValidation_remittanceInformationStructuredArray_referenceType_error() {
    ...
    @Test
    void doSingleValidation_remittanceInformationStructured_error() {
        Remittance remittance = new Remittance();
        remittance.setReference(VALUE_36_LENGHT);
        remittance.setReferenceType(VALUE_71_LENGHT);
        remittance.setReferenceIssuer(VALUE_141_LENGTH);
        singlePayment.setRemittanceInformationStructured(remittance);

        validator.doSingleValidation(singlePayment, messageError, validationConfig);
        assertEquals(MessageErrorCode.FORMAT_ERROR_OVERSIZE_FIELD, messageError.getTppMessage().getMessageErrorCode());
        assertArrayEquals(new Object[]{"referenceIssuer", 140}, messageError.getTppMessage().getTextParameters());
    }
}
