...
This interface will be called instead of other payment SPI interfaces for any payment product that doesn't belong to the pre-defined list of standard JSON payment products, regardless of payment service or content type.
...
*getPaymentStatusById* method returns a *SpiGetPaymentStatusResponse* object (with the transaction status) and has the following parameters:
