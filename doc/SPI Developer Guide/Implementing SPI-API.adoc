...
SPI-API is used for connection between XS2A interface and core-banking, that's why this system is also called `connector`
...
- Provide this data to the core-banking system (and/or add some business logic, if necessary)
- Retrieve the response from core-banking
- Provide the core-banking response back to XS2A using `SpiResponse` class
...
- `SpiAspspConsentDataProvider` - this service is used for providing ASPSP consent data to connector and core-banking
