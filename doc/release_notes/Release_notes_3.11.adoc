...
From now on, trying to update PSU Data for AIS authorisation (`PUT /v1/consents/{consentId}/authorisations/{authorsationId}`)
with `FAILED` authorisation status will result in `409 STATUS_INVALID` error being returned to the TPP.
