...
- first, it has to be a string representation of a day of the month (1-31), violating this returns `400 FORMAT_ERROR`;
- second, its value should be higher than number of current day of month (in other words, it should be in the future).
Violating this returns `400 EXECUTION_DATE_INVALID` response.
