...
In case PSU decides to close an account in the bank - ASPSP enables to revoke all AIS and PIIS consents of account in one step. It can be performed via endpoint in the CMS.
...
During Consent creation in request the additional account information for AIS as *owner name* can be asking.
...
Fields *"availableAccounts"*, *"availableAccountsWithBalance"* and *"allPsd2"* may have values *"allAccounts"* and *"allAccountsWithOwnerName"*.
So if any of these fields are present in consent request and ASPSP-Profile contains *accountOwnerInformationSupported=true*, new values should be stored in consent.
