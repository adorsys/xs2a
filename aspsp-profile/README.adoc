...
If parameter = FALSE then authorisation is supported by usual flow. if it is TRUE - each authorisation should be confirmed. |FALSE |TRUE, FALSE

|authorisationConfirmationCheckByXs2a |This field indicates which part of application is responsible for authentication data checking in the authorisation process.

If parameter = FALSE then it is SPI developer's duty to verify the authentication data. If it is TRUE then verification of this data is processed on XS2A side. |FALSE |TRUE, FALSE
