...
public class RoleOfPSP {
	...
	public static RoleOfPSP getInstance(ASN1Encodable asn1Encodable) {
		...
		if (RoleOfPspOid.id_psd2_role_psp_as.getId().equals(objectIdentifier.getId())
				&& RoleOfPspName.PSP_AS.getString().equals(instance.getString())) {//NOPMD
			return PSP_AS;
		}
		if (RoleOfPspOid.id_psd2_role_psp_pi.getId().equals(objectIdentifier.getId())
				&& RoleOfPspName.PSP_PI.getString().equals(instance.getString())) {//NOPMD
			return PSP_PI;
		}
		if (RoleOfPspOid.id_psd2_role_psp_ai.getId().equals(objectIdentifier.getId())
				&& RoleOfPspName.PSP_AI.getString().equals(instance.getString())) {//NOPMD
			return PSP_AI;
		}
		if (RoleOfPspOid.id_psd2_role_psp_ic.getId().equals(objectIdentifier.getId())
				&& RoleOfPspName.PSP_IC.getString().equals(instance.getString())) {//NOPMD
			return PSP_IC;
		}
